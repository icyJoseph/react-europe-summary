...
Being able to `throw` in JavaScript enables <br/>
`Suspense` and opens the door for `Concurrent` mode.
...
- Concurrent mode (basically) throws while rendering.
- Skipping the commit phase, to do something more important.
...
- Unstable for now
- React 16.x (~Q2 2019) ?
- Lead facebook to contribute `isInputPending` to the browser API.
...
- Support a render phase without a commit one
  - In concurrent mode last render doesn't necessarily mean "latest committed state".

---

Totally not the end

- `createContext` without `useMemo` updates every render, and reloads the whole subtree.
  - React has no way to bailout on Context updates (try to do before Monday)
- `useContext` + `useReducer` + `useMemo` = naive `Redux` (try to do before Monday)
- Measuring Height
- `hidden` prop
- Designing a component style API
